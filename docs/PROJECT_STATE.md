...
33.8%, which is chance. Seven corridors are digitised in several pieces, so **153 polygons
...
corridors *to each other*, and its slope is 0.53. Across summers, the same two bands
...
Radar rather than NDVI because NDVI is an input to the index, so residualising on it
...
⚠️ `docs/preprint/` is **gitignored on purpose** (`.gitignore:65`), because an unposted
