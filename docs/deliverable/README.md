...
**The `recommendation` column is the City's text, not mine.** It is reproduced
verbatim from Appendix J of the City of Surrey's *Biodiversity Conservation
Strategy* ([Appendix_J.pdf](https://www.surrey.ca/sites/default/files/media/documents/Appendix_J.pdf),
pages 119–129 of the parent report), parsed by `scripts/parse_appendix_j.py`.
The same source supplies `ecological_value`, `risk_of_development`,
`corridor_type` and `target_width_m`, which the City also publishes as
attributes on its open-data GIS layer. Copyright in all of it rests with the City
of Surrey, and it is reproduced here under attribution because the ranking is not
usable by a planner without it. The rest of these files is mine and carries the
repository's licence: the CDEI values, the ranks, the percentiles, the priority
flag.
