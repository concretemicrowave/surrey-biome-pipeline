...
Identity, data and status colours are kept disjoint. A colour that carries two
of those roles carries neither legibly, so the accent never appears in the data
palette even though it would look fine there, and the warning red is never a
series.
"""

from __future__ import annotations

# Same values as the manuscript preamble (accent, accentpale in
# docs/papers/paper-a/main.tex), so the paper, its figures and the board are
# recognisably one project.
ACCENT = "#1f4e5f"
ACCENT_PALE = "#eef3f5"
...
SERIES = "#2a78d6"
CRITICAL = "#d03b3b"
# Sequential, not a rainbow: stress is a magnitude, and a rainbow would imply
# category boundaries the percentile does not have.
STRESS_CMAP = "Reds"
...
# canopy-density map, was tested and did not hold (Spearman rho = +0.924 after
# residualising against VH backscatter), so this is the weaker and accurate
# claim, matching the manuscript.
EXPLORATORY_NOTE = (
