...
The dry edge is fit **across all polygon-summers at once**, not per year: a
...
def frozen_reference(path: Path) -> tuple[tuple[float, float], pd.Series]:
    ...
    logger.info("frozen reference from %s: edge SWCI = %.4f + %.4f * NDVI, "
                "RLST baseline over %d polygons x summers %s",
...
def compute_tvwsi(df: pd.DataFrame, *, n_bins: int = 12, q: float = 0.05,
    ...
    if baseline is None:
        ...
        if base.isna().any():
            ...
            raise ValueError(f"{len(missing)} polygons absent from the frozen "
...
def main() -> None:
    ...
    print("=" * 70)
    print(f"Feature table: {len(df)} rows | {df[CORRIDOR_ID].nunique()} polygons "
