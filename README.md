...
[![Preprint](https://img.shields.io/badge/preprint-10.32942%2FX2ZH5C-blue.svg)](https://doi.org/10.32942/X2ZH5C)
...
degraded* model was better than the scale-free one on forest-stand water stress
in every configuration tested (every cell size, all 20 blocking seeds, all
eight forest settings), but the paired interval spans zero, so the difference is
a consistent direction rather than a resolved effect. Fine spatial detail in the
climate predictors bought nothing. Water stress at this scale is organised by
local terrain and stand composition, not by the macroclimate gradient.

This repository is the code, method and applied output. The full write-up is a
preprint, posted to EcoEvoRxiv on 2026-08-26 as
[10.32942/X2ZH5C](https://doi.org/10.32942/X2ZH5C) under CC BY 4.0. **A preprint
has not been peer reviewed**, so read it as a manuscript rather than as a
published result.
[`docs/RESOLUTION_TEST_FINDINGS.md`](docs/RESOLUTION_TEST_FINDINGS.md) is the working
account the preprint was written from, and it keeps the run-level detail the
manuscript has no room for.
...
- **Model A:** ClimateBC values queried at each polygon's own location and elevation, downscaled from a 4 km baseline.
...
| Quantity | Value |
|---|---|
| Paired ΔRMSE (RMSE_A − RMSE_B), transect @ 25 km | **+0.00093**, 95% CI [−0.00061, +0.00257] |
...
| Coarse model better on the point estimate | 32 of 32 runs, across cell sizes, seeds and forest settings |
| Paired interval excludes zero | 1 of 32, only the least constrained forest |
...
- The label depends on the cell size. The verdict rule requires
  ...
  but misses it by three thousandths at 12 km. So `seed_sensitivity.py 25000`
  prints `NOT SUPPORTED` 20 times and `seed_sensitivity.py 12000` prints
  `INCONCLUSIVE` 20 times, while the paired ΔRMSE is +0.00093 and +0.00094
...
- Significance is almost entirely absent, and depends on how hard the forest is
  allowed to overfit. Across eight hyperparameter settings the coarse model is
  better on the point estimate in all eight, but the paired interval excludes
  zero in only one, `min_samples_leaf = 1`, the least constrained forest and
  the one where Model A overfits hardest. Constraining the forest closes the gap
  by driving both models toward the mean: Model A's deficit is substantially
  overfitting to fine climate detail.
...
"falsified" did, and it did not survive. An August 2026 correction to the
paired confidence interval (recorded at the top of
[`docs/RESOLUTION_TEST_FINDINGS.md`](docs/RESOLUTION_TEST_FINDINGS.md)) found
that it
had been bootstrapped over 25 folds that reuse the same rows, five re-splits of
one panel rather than five samples. Resampling only within a repeat widens the
interval about 2.1× and it no longer excludes zero anywhere. The direction
survived every sensitivity analysis; the significance did not survive the fix.
...
![CDEI feature space: 612 polygon-summers plotted by greenness against canopy
water, with the fitted dry edge](docs/figures/cdei_feature_space.png)

*All 612 polygon-summers, with the dry edge as `assemble.dry_edge` actually fits
...
```bash
uv venv --python 3.12
uv sync --locked --extra dev   # [dev] adds jupyter, pytest, ruff, matplotlib
```

`--locked` is the point: `pyproject.toml` declares floors (`geopandas>=1.0`),
while `uv.lock` pins all 221 transitive packages with hashes. Installing with
`uv pip install -e .` resolves fresh against PyPI instead, so it reproduces the
dependency *declaration* and not the environment the published numbers came out
of. Use `--locked` unless you are deliberately testing against newer releases.
...
> 153-polygon × 5-period pass is 765 calls, about 16 hours. The transect's
...
All public, all free: City of Surrey Open Data (GIN corridors); Appendix J of the
City of Surrey's *Biodiversity Conservation Strategy* (corridor attributes and the
written management recommendations); BC Data Catalogue (VRI, BEC); ClimateBC
(`api6.climatebc.ca`); Sentinel-1/2 and Landsat 8/9 via public STAC catalogues
(Element84 Earth Search and Microsoft Planetary Computer, with fallback between
them); Copernicus GLO-30 DEM.

Raw and intermediate data are not redistributed here. The one exception is
Appendix J's corridor attributes and recommendation text, which are reproduced in
`docs/deliverable/corridor_stress_ranking.csv` because the ranking is unusable to
a planner without them. That text is the City's, is copyright the City of Surrey,
and is included under attribution; see
[`docs/deliverable/README.md`](docs/deliverable/README.md) for the exact columns
and the source PDF.

## Data availability

Every input is public and free, and the acquisition code that fetches it is in
this repository (`src/pipeline/acquire_*.py`). Nothing here requires a licensed
dataset or a credentialed endpoint.

What is **not** committed is the raw and intermediate data itself
(`data/raw`, `data/interim`, `data/processed`), which is gitignored. Rasters and
vectors are large, and every one of them is redistributable from its source by
running the pipeline. The one costly step is the ClimateBC pull: the web API
allows roughly 50 calls per hour per IP, so a full acquisition runs for many
hours. It is paced, resumable and permanently disk-cached, so an interrupted run
resumes for free.
...
| If you want | Read |
|---|---|
| The argument, written up | [the preprint, 10.32942/X2ZH5C](https://doi.org/10.32942/X2ZH5C) |
...
validation. The manuscript is posted as a preprint,
[10.32942/X2ZH5C](https://doi.org/10.32942/X2ZH5C), and has **not** been peer
reviewed, so nothing here should be described as published. The Surrey corridor
ranking is exploratory pending ground confirmation.
...
results and any errors that may remain.

## License

Code is released under the MIT License (see [`LICENSE`](LICENSE)). Documentation and
figures under `docs/` are © the author, with one carve-out: the corridor
attributes and recommendation text reproduced from the City of Surrey's Appendix J
are the City's, not mine, and neither the MIT licence nor my copyright extends to
them. They are identified column by column in
[`docs/deliverable/README.md`](docs/deliverable/README.md) and stated at the root
in [`NOTICE`](NOTICE), which sits beside `LICENSE` so the carve-out is visible to
anyone who reads the licence without reading the docs. `LICENSE` itself is left
as unmodified MIT text so automated licence detection still identifies it.
